...
- **Process spawn:** `@tauri-apps/plugin-shell` `Command.create(program, args).execute()/.spawn()`; **capability-gated** — the default permission set only allows opening `http(s)://`, `tel:`, `mailto:` links; running a command requires explicit `shell:allow-execute` / `shell:allow-spawn` scope entries, which can pin `name`/`cmd`/`args` (with arg validators) [S25]. Confidence: High (page re-read today; permission table and default-permission block read verbatim). Note: the plugin does **not** hand the caller a native process handle, so Job Object containment (§7) is implemented in Rust command handlers, not via the JS plugin.
